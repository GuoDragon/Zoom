...
4. 严格按照已经定义好的数据结构`app\src\main\java\com\example\Zoom\model`和已有的数据`app\src\main\assets\data`
...
## 当前任务
1. 参考已经完成的相似功能 app（`D:\AndroidStudio\app\android_template\Tencent-Meeting`），注意先只关注页面开发的相关内容
2. 依据`app\src\main\java\com\example\Zoom\model`里面的数据结构，设计一些初始的数据。
3. 数据存储在`app\src\main\assets\data`下。