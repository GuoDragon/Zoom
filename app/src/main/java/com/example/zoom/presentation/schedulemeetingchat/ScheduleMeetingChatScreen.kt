...
@Composable
fun ScheduleMeetingChatScreen(
    meetingId: String,
    onBackClick: () -> Unit
) {
    var uiState by remember { mutableStateOf<ScheduleMeetingChatUiState?>(null) }
    var localMessages by remember { mutableStateOf<List<ScheduleMeetingChatMessageUi>>(emptyList()) }
    var inputText by remember { mutableStateOf("") }
    val listState = rememberLazyListState()

    val view = remember {
        object : ScheduleMeetingChatContract.View {
            override fun showContent(content: ScheduleMeetingChatUiState) {
                uiState = content
                localMessages = content.messages
            }
        }
    }
    val presenter = remember(view) { ScheduleMeetingChatPresenter(view) }
    ...
    val title = uiState?.meetingTitle ?: "Meeting chat"
    ...
    Scaffold(
        ...
        Column(
            ...
            Row(
                ...
                IconButton(
                    onClick = {
                        presenter.sendMessage(meetingId = meetingId, content = inputText)?.let { newMessage ->
...
@Composable
private fun MessageBubble(message: ScheduleMeetingChatMessageUi) {
    Column(
        modifier = Modifier.fillMaxWidth(),
        horizontalAlignment = if (message.isSelf) Alignment.End else Alignment.Start
    ) {
        Text(
            text = if (message.isSelf) "You · ${message.timestampLabel}" else "${message.senderName} · ${message.timestampLabel}",
